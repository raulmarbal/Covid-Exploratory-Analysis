...
-- Total cases vs Total deaths (death percentage of covid globally)
-- For Tableau Viz - 1
...
-- For Tableau Viz - 2
SELECT location, SUM(CAST(new_deaths as int)) AS total_deaths -- Issue with total_deaths column, need to put cast as int
FROM covid_project..covid_deaths$
...
-- Countries with highest rate of cases compared to population
-- For Tableau Viz - 4
SELECT location, population, date, MAX(total_cases) AS max_cases, MAX(total_cases/population)*100 AS max_cases_percentage
FROM covid_project..covid_deaths$
WHERE continent IS NOT NULL
